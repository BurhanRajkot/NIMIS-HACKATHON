...
def analyze_address():
    ...
    # Initialize parser to check for embedded city in address
    parser = AddressParser()
    parsed = parser.parse(raw_address)

    # Determine city:
    # 1. Explicitly passed in request (priority)
    # 2. Detected from address text
    # 3. Default from config
    request_city = data.get('city', '').strip()
    ...
    try:
        ...
        # Step 1: Parse address text (Already done above for city detection)
        # parsed = parser.parse(raw_address)
